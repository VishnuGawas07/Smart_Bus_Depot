...
class Addtpassdata {
    ...
    static int save(String idno, String passno, String name, String price, String issuedate, String validupto, String dob, String age, String source, String destination, String address, String institute) {
        ...
        try{
            ...
            PreparedStatement ps=con.prepareStatement("insert into touristpass(idno,passno,name,price,issuedate,validupto,dob,age,source,destination,address,institute)values(?,?,?,?,?,?,?,?,?,?,?,?)");
            ps.setString(1,idno);
     ...
     public static int delete(int id){
        ...
        try{
            Class.forName("oracle.jdbc.driver.OracleDriver");
            Connection con=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","system","123");
            PreparedStatement ps=con.prepareStatement("delete from touristpass where passno=?");
            ps.setInt(1,id);
            status=ps.executeUpdate();
            con.close();
        }catch(Exception e){System.out.println(e);}
